"""Emit LLMOSES run state through Python-owned file I/O.

The MeTTa loop calls this module to append native audit records, write per-run
state/action files, and drop ready sentinels after each generation. new_run()
opens state/run-<seq>/ and action/run-<seq>/ subdirectories so multiple
runMoses calls in one process do not collide. The watcher owns utilities/ and
traces/; this emitter only records native state and signals readiness.
"""
import os
import json
import time
...
# --- path resolution -------------------------------------------------------
# Combined human log. Override with LLMOSES_STATE_LOG.
...
def emit_gen(gen_index, remaining, pop_size, best_score, deme_size):
    ...
    # Channel 3: ACTION, the action space available this
    # gen. The action actually TAKEN (which exemplar selectExemplar chose) lives
    # inside expandDeme and needs a trace hook.
    action_line = (
    ...
    _NFH.write(json.dumps({
        ...
        "action_taken":               None,   # action hook not emitted here
